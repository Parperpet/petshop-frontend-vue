<template>
  <div>
    ...
    <center>
      <div class="container">
        <h1 class="mt-3">Contato com o dono do pet</h1><br>
        <div class="row">
          <div class="col-6">          
            <img v-if="usuario.fotoPerfilBase64 != 0" :src="pet.dono.fotoPerfilBase64"  class="img-thumbnail" />            
            <img v-else src="@/assets/sem-foto.jpg"  class="img-thumbnail" />
            <h4 class="ml-3">Dono: {{pet.dono.nome}}</h4>
            <h4 class="ml-3">Email: {{pet.dono.email}}</h4>
            <h4 class="ml-3">Telefone: {{pet.dono.telefone}}</h4>
          </div>  
          <div class="col-6">          
 ...
 components:{
  ...
  methods:{
    enviarMensagem(){
      ...
      this.mensagem.destinatario = this.pet.dono
